...
class AddProduct extends React.Component {
    ...
    postBrand = async () => {
        ...
        if (resp.data && !resp.data.succes) {
            this.setState({ error: resp.data.error }, () => setTimeout(() => this.setState({ error: '' }),2000))
        }
    }
    postCategory = async () => {
    ...
    handleSave = async () => {
        ...
        try {
            ...
            this.setState({ error: ex }, () => setTimeout(() => this.setState({ error: '' }),2000))
        }
    }

    modal = () => {
        const { category, brand } = this.props
        const { openModal, selectedCategory, selectedBrand, productName, error } = this.state
        return (
